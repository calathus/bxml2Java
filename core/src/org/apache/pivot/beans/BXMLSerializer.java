...
public class BXMLSerializer implements Serializer<Object>, Resolvable {
    static boolean isOperatorElement(Element.Type type) {
        return type == Element.Type.WRITABLE_PROPERTY
                || type == Element.Type.READ_ONLY_PROPERTY
                || type == Element.Type.LISTENER_LIST_PROPERTY;
    }
    static boolean isDefineElement(Element.Type type) {
        return type == Element.Type.INCLUDE
    ...
    private BXMLSerializer parentBXMLSerializer;
    ...
    public BXMLSerializer(ICodeEmitter codeEmitter) {
    ...
    // [nn]
    public void setCodeEmitter(ICodeEmitter codeEmitter) {
        this.codeEmitter = codeEmitter;
    }
    public void setParentBXMLSerializer(BXMLSerializer parentBXMLSerializer) {
        this.parentBXMLSerializer = parentBXMLSerializer;
    }

    private boolean isEmitMode() {
        return codeEmitter != null;
    }
    ...
    public Object readObject(InputStream inputStream)
        ...
        if (isEmitMode()) codeEmitter.code_start(); // [nn]
                ...
                while (xmlStreamReader.hasNext()) {
                    ...
                    switch (event) {
                        ...
                        case XMLStreamConstants.START_ELEMENT: {
                            if (isEmitMode()) codeEmitter.inc();
                            processStartElement();
                            if (isOperatorElement(element.type)) {
                                if (isEmitMode()) codeEmitter.dec();
                            }
                            break;
                        }

                        case XMLStreamConstants.END_ELEMENT: {
                            Element.Type type0 = element.type;
                            processEndElement();
                            if (!isOperatorElement(type0)) {
                                if (isEmitMode()) codeEmitter.dec();
                            }
                            break;
                        }
        ...
        if (isEmitMode()) codeEmitter.code_end(); // [nn]
    ...
    private void processStartElement() throws IOException, SerializationException {
        ...
        if (prefix != null
            ...
            if (Character.isUpperCase(localName.charAt(0))) {
                ...
                if (isEmitMode()) codeEmitter.code_comment(name, ""+elementType);
            }
        }
        ...
        if (elementType == Element.Type.INCLUDE) {
            ...
            InputStream inputStream = new BufferedInputStream(location.openStream());
            try {
                if (serializer instanceof BXMLSerializer) {
                    BXMLSerializer bxmlSerializer = (BXMLSerializer)serializer;
                    bxmlSerializer.setCodeEmitter(codeEmitter);
                    bxmlSerializer.setParentBXMLSerializer(this);
                }
            ...
            System.out.println(">>> REFERENCE name: "+element.name+", id: "+id); // [nn]
        ...
        for (int i = 0, n = xmlStreamReader.getAttributeCount(); i < n; i++) {
            ...
            if (prefix != null
                ...
                if (localName.equals(ID_ATTRIBUTE)) {
                    ...
                    element.id = value;
                    // [nn]
                    if (isEmitMode() && (element.type != Element.Type.INCLUDE)) {
                        //System.out.println("ID_ATTRIBUTE===>> : prefix: "+prefix+", name: "+value);
                        codeEmitter.code_node_id(value);
                   }
                } else {
                                ...
                                if (value.length() > 0) {
                                    if (value.charAt(0) == OBJECT_REFERENCE_PREFIX) {
                                        ...
                                        if (value.equals(BXML_PREFIX + ":" + null)) {
                                            ...
                                            if (isEmitMode()) codeEmitter.register_reference(value, attribute.value);
                                        }
                                    }
                                } else {
            ...
            if (bindingAnnotation != null) {
                ...
                if (namespace.containsKey(id)) {
                    ...
                    try {
                        ...
                        // [nn]
                        if (isEmitMode()) codeEmitter.code_bind(field, object, value);
                    } catch (IllegalAccessException exception) {
    ...
    protected Serializer<?> newIncludeSerializer(Class<? extends Serializer<?>> type)
        ...
        Serializer<?> obj = type.newInstance();
        return obj;
    }
    ...
    private void setStaticProperty(Object object, Class<?> propertyClass,
        ...
        try {
            ...
            // [nn]
            if (isEmitMode()) codeEmitter.code_static_set_attr(setterMethod, object, value);
